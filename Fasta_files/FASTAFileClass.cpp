...
#include "FASTAFileClass.h"


namespace FastaFile {


    FASTAFileClass::FASTAFileClass() = default; //default constructor.

    FASTAFileClass::FASTAFileClass(std::string &file_name) // Constructor when using a .Fa File as parameter.
    {
        ...
        DNA_sequence::SequenceClass sequence_obj; //Prepare a Sequence Object.
        bool last = false;
        ...
        if (check_pass) {
            ...
            std::list<DNA_sequence::SequenceClass> list_of_Sequences; //Prepare a list to save every instance of Sequences found.
            ...
            while (getline(basicIfstream, lineIN_string)) {
                ...
                if (check_secuencia == '>') {
                    ...
                    DNA_sequence::SequenceClass sequence_INobj; //Prepare a new Sequence Object to load information.
                    sequence_INobj.assignName(lineIN_string.substr(1)); //Assign name to the Sequence.
                    ...
                    while (getline(basicIfstream, reading_string_line)) {
                        ...
                        if (check_secuencia != '>' &&
                            ...
                            if (sequence_INobj.sequenceCorrect() &&
                                ...
                                sequence_INobj.makeGraph(); //Creates the Graph for the Sequence.
    ...
    std::string FASTAFileClass::NombreArchivo() { //Simple Get Function.
        return this->file_name_;
    }

    FASTAFileClass::~FASTAFileClass() { //Destructor Function.
        this->file_name_.clear();
        this->sequences_list_.clear();
    }

    void FASTAFileClass::printInformation() { //Simple information Printer.
    ...
    void FASTAFileClass::exportLegible() { //Human readable .fa file export.
        ...
        std::list<DNA_sequence::SequenceClass>::iterator it; // Iterator of the list of Sequences.
    ...
    int FASTAFileClass::isSubSequence(std::string sub_sequence) {
        if (this->empty_file_) {
            return 0;
        }
        int contador = 0;
        typename std::list<DNA_sequence::SequenceClass>::iterator it;
    ...
    void FASTAFileClass::maskFile(const std::string &to_mask) {
        maskFile(to_mask, "X"); //Mask (replace) every Base (o combination) to "X" default Base.
    }

    void FASTAFileClass::maskFile(const std::string &to_mask, const std::string &mask) { //Implementation.
        typename std::list<DNA_sequence::SequenceClass>::iterator it;
        std::list<std::string>::iterator its;
    ...
    void FASTAFileClass::HuffmanEncodder() {
        HuffmanEncodder(true);
    }

    void FASTAFileClass::HuffmanEncodder(bool Mask) {
        ...
        Huffman huff;
        huff.HuffmanEncoding(char_array, freq_array, int(sizeof(char_array) / sizeof(char_array[0])));
    ...
    std::map<char, int> FASTAFileClass::freqMapping() {
    ...
    void FASTAFileClass::compressFile(std::string file_name) {
    ...
    FASTAFileClass::FASTAFileClass(std::string &file_name, const int &bin_opcion) {
        file_name = prepareFileName(file_name, ".fabin");
        ...
        Huffman huff_2;
        huff_2.HuffmanEncoding(char_array, freq_array, int(sizeof(char_array) / sizeof(char_array[0])));
        int32_t seq_count = 0;
        infile.read((char *) &seq_count, sizeof(seq_count));
        std::list<DNA_sequence::SequenceClass> list_secuencias_in;
        typename std::list<DNA_sequence::SequenceClass>::iterator it_ls;
        this->DNAsequences_count = seq_count;
        for (int j = 1; j <= seq_count; j++) {
            DNA_sequence::SequenceClass sequence_obj_in;
    ...
    std::string FASTAFileClass::prepareFileName(std::string &file_name, const std::string &extension) {
    ...
    void FASTAFileClass::FastaGraphmaker() {
        for (auto &it: this->sequences_list_) {
            FastaGraph::GraphClass::makeFastaGraph(it);
        }
    }
